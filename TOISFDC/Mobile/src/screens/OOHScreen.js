/**
 * OOH Screen — Professional UI rewrite
...
const CATEGORY_OPTIONS = [
  '--None--', 'Retail & Service', 'Educational Institution', 'Corporate Office',
  'Healthcare', 'Entertainment', 'Transport Hub', 'Hospitality', 'Other',
];
const SUB_CATEGORY_OPTIONS = [
  '--None--', 'Office Campuses', 'Shopping Mall', 'Hospital', 'School',
  'College', 'Hotel', 'Restaurant', 'Railway Station', 'Airport', 'Other',
];
const TYPE_OPTIONS = [
  '--None--', 'Music And Book', 'Grocery', 'Medical', 'Stationery',
  'Pharmacy', 'Bakery', 'Garments', 'Other',
];
const MEASURE_OPTIONS = [
  '--None--', 'No of Stores', 'No of Offices', 'No of Students',
  'No of Beds', 'No of Rooms', 'No of Seats', 'Other',
];
...
function emptyForm() {
  return {
    ...
    type: '--None--', measure_of_potential: '--None--',
    address: '', potential_count: '',
    depot_id: '', depot_name: '',
...
// ── Main Screen ───────────────────────────────────────────────────────
export default function OOHScreen() {
  ...
  async function handleSearch() {
    ...
    setSearching(true);
    try {
      const result = await displaySegmentsOOH(searchText);
      setSegments(result || []);
      setView('list');
    } catch (e) {
  ...
  async function doCheckIn() {
    ...
    try {
      ...
      const result   = await checkInSegment({
        ...
        segType: 'OOH', mobId: mobileId, deviceNM: getDeviceName(),
      });
      if (result.success) {
  ...
  async function doCheckOut() {
    ...
    try {
      ...
      const result = await checkOutSegment({
        lat: pos.coords.latitude, lng: pos.coords.longitude,
        recordId: selected.id || selected.Id, segType: 'OOH',
      });
      if (result.success) {
  ...
  async function handleCreate() {
    ...
    setSaving(true);
    try {
      const result = await createSegmentOOH({ ...form, created_by: userId });
      if (result.success) {
        Toast.show({ type: 'success', text1: 'OOH record created successfully.' });
        setView('search');
        setSearchText('');
        setSegments([]);
      } else {
  ...
  function openUpdate() {
    setForm({
      ...
      sub_category:         selected.sub_category || '--None--',
      type:                 selected.type || '--None--',
      measure_of_potential: selected.measure_of_potential || '--None--',
      address:              selected.address || '',
      potential_count:      selected.potential_count?.toString() || '',
  ...
  async function handleUpdate() {
    ...
    setSaving(true);
    try {
      const result = await updateSegmentOOH(selected.id || selected.Id, form);
      if (result.success) {
        Toast.show({ type: 'success', text1: 'OOH record updated successfully.' });
        setView('search');
        setSegments([]);
        setSearchText('');
      } else {
    ...
    <KeyboardAvoidingView style={s.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <View style={s.searchArea}>
        <View style={s.searchRow}>
          <View style={s.searchBox}>
            ...
            <TextInput
              style={s.searchInput}
              placeholder="Search OOH locations, depots..."
      ...
      <View style={s.emptyState}>
        <View style={[s.emptyIconBox, { backgroundColor: LIGHT }]}>
          <MaterialIcons name="campaign" size={44} color={ACCENT} />
        </View>
        <Text style={s.emptyTitle}>Search OOH Records</Text>
      ...
      <FlatList
          ...
          <TouchableOpacity style={[s.card, { borderLeftColor: ACCENT }]} onPress={() => handleSelectRecord(item)} activeOpacity={0.8}>
            <View style={s.cardHeader}>
              <View style={[s.cardIconBox, { backgroundColor: LIGHT }]}>
                <MaterialIcons name="campaign" size={20} color={ACCENT} />
              </View>
            ...
            <View style={s.chipRow}>
              ...
              {!!item.category && <InfoChip icon="category" label={item.category} accent={ACCENT} />}
            </View>
        ...
        ListEmptyComponent={
          <View style={s.noResults}>
            ...
            <Text style={s.noResultsText}>No OOH records found</Text>
          </View>
        }
      />
    </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.detailHero, { borderTopColor: ACCENT }]}>
          <View style={[s.detailHeroIcon, { backgroundColor: LIGHT }]}>
            <MaterialIcons name="campaign" size={32} color={ACCENT} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={s.detailHeroName}>{selected?.name_of_society || '—'}</Text>
            <View style={s.chipRow}>
              {!!selected?.category && <InfoChip icon="category" label={selected.category} accent={ACCENT} />}
              {!!selected?.sub_category && <InfoChip icon="label" label={selected.sub_category} accent={ACCENT} />}
              {!!selected?.type && <InfoChip icon="local-offer" label={selected.type} accent={ACCENT} />}
            </View>
          </View>
        ...
        <View style={s.detailCard}>
          ...
          <SFRow L="Branch" LV={selected?.branch_name} R="" RV="" />
        </View>

        {/* Section: Classification */}
        <View style={s.detailCard}>
          <View style={[s.detailCardBar, { backgroundColor: ACCENT }]}>
            <MaterialIcons name="category" size={13} color="#fff" style={{ marginRight: 5 }} />
            <Text style={s.detailCardBarText}>CLASSIFICATION</Text>
          </View>
          <SFRow L="Category" LV={selected?.category} R="Sub Category" RV={selected?.sub_category} />
          <SFRow L="Type" LV={selected?.type} R="Measure of Potential" RV={selected?.measure_of_potential} />
          <SFRow L="Potential Count" LV={selected?.potential_count?.toString()} R="Premiumness" RV={selected?.premiumness} />
        </View>
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[s.formTopBar, { backgroundColor: ACCENT }]}>
        ...
        <Text style={s.formTopBarText}>{isUpdate ? 'Edit OOH Record' : 'New OOH Record'}</Text>
      </View>
      <ScrollView style={s.page} keyboardShouldPersistTaps="handled">
        <View style={s.formBody}>
          ...
          <TextFieldInput label="Premiumness" value={form.premiumness} onChangeText={v => setField('premiumness', v)} keyboardType="numeric" placeholder="e.g. 2" />
          <TextFieldInput label="Address" required value={form.address} onChangeText={v => setField('address', v)} multiline />

          <Text style={[s.formSectionLabel, { color: ACCENT }]}>Classification</Text>
          <DropdownPicker label="Category" value={form.category} options={CATEGORY_OPTIONS} onSelect={v => setField('category', v)} />
          <DropdownPicker label="Sub Category" value={form.sub_category} options={SUB_CATEGORY_OPTIONS} onSelect={v => setField('sub_category', v)} />
          <DropdownPicker label="Type" value={form.type} options={TYPE_OPTIONS} onSelect={v => setField('type', v)} />
          <DropdownPicker label="Measure of Potential" value={form.measure_of_potential} options={MEASURE_OPTIONS} onSelect={v => setField('measure_of_potential', v)} />
          <TextFieldInput label="Potential Count" value={form.potential_count} onChangeText={v => setField('potential_count', v)} keyboardType="numeric" />
