/**
 * RWA Screen — Professional UI rewrite
...
// ─────────────────────────────────────────────
// Picklist values (mirrors Salesforce metadata)
// ─────────────────────────────────────────────
const CATEGORY_OPTIONS  = ['--None--', 'A+', 'A', 'B', 'C', 'D'];
const CITY_OPTIONS      = ['--None--', 'Mumbai', 'Delhi', 'Noida', 'Gurugram',
                           'Bengaluru', 'Hyderabad', 'Chennai', 'Kolkata',
                           'Pune', 'Ahmedabad', 'Jaipur', 'Haryana', 'Rajasthan'];
const DELIVERY_OPTIONS  = ['--None--', 'Door Delivery', 'Bulk Delivery', 'Hawker', 'Counter Sale', 'Home Delivery'];
...
function emptyForm() {
  return {
    name_of_society: '', address: '',
    slab: '', premiumness: '',
    category: '--None--', city: '--None--', delivery_mode: '--None--',
...
export default function RWAScreen() {
  ...
  // view: 'search' | 'list' | 'create' | 'detail' | 'update'
  ...
  // Form state (create & update share the same form)
  ...
  async function handleSearch() {
    ...
    setSearching(true);
    try {
      const result = await displaySegmentsRWA(searchText);
      setSegments(result || []);
      setView('list');
    } catch (e) {
  ...
  async function doCheckIn() {
    ...
    try {
      ...
      const result   = await checkInSegment({
        ...
        segType: 'RWA', mobId: mobileId, deviceNM: getDeviceName(),
      });
      if (result.success) {
  ...
  async function doCheckOut() {
    ...
    try {
      ...
      const result = await checkOutSegment({
        lat: pos.coords.latitude, lng: pos.coords.longitude,
        recordId: selected.id || selected.Id, segType: 'RWA',
      });
      if (result.success) {
  ...
  async function handleCreate() {
    if (!form.name_of_society.trim()) return Toast.show({ type: 'error', text1: '* Name of Society is required.' });
    ...
    setSaving(true);
    try {
      const result = await createSegmentRWA({ ...form, created_by: userId });
      if (result.success) {
        Toast.show({ type: 'success', text1: 'RWA record created successfully.' });
        setView('search');
        setSearchText('');
        setSegments([]);
      } else {
  ...
  function openUpdate() {
    setForm({
      ...
      city:             selected.city || '--None--',
      delivery_mode:    selected.delivery_mode || '--None--',
      ...
      total_flats:      selected.total_flats?.toString() || '',
      occupied_flats:   selected.occupied_flats?.toString() || '',
    });
    setView('update');
  }
  ...
  async function handleUpdate() {
    ...
    setSaving(true);
    try {
      const result = await updateSegmentRWA(selected.id || selected.Id, form);
      if (result.success) {
        Toast.show({ type: 'success', text1: 'RWA record updated successfully.' });
        setView('search');
        setSegments([]);
        setSearchText('');
      } else {
  ...
  // ── Depot / Locality search helpers ─────────
  async function handleDepotSearch(q) {
    setDepotLoading(true);
    try {
      const res = await searchDepots(q);
      setDepotResults(res || []);
    } catch {} finally { setDepotLoading(false); }
  }

  async function handleLocalitySearch(q) {
    setLocalityLoading(true);
    try {
      const res = await searchLocalities(q);
      setLocalityResults(res || []);
    } catch {} finally { setLocalityLoading(false); }
  }
    ...
    <KeyboardAvoidingView style={s.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <View style={s.searchArea}>
        <View style={s.searchRow}>
          <View style={s.searchBox}>
            ...
            <TextInput
              style={s.searchInput}
              placeholder="Search societies, localities..."
      ...
      <View style={s.emptyState}>
        <View style={[s.emptyIconBox, { backgroundColor: LIGHT }]}>
          <MaterialIcons name="apartment" size={44} color={ACCENT} />
        </View>
        <Text style={s.emptyTitle}>Search RWA Records</Text>
      ...
      <FlatList
          ...
          <TouchableOpacity style={[s.card, { borderLeftColor: ACCENT }]} onPress={() => handleSelectRecord(item)} activeOpacity={0.8}>
            <View style={s.cardHeader}>
              <View style={[s.cardIconBox, { backgroundColor: LIGHT }]}>
                <MaterialIcons name="apartment" size={20} color={ACCENT} />
              </View>
              <Text style={s.cardName} numberOfLines={2}>{item.name_of_society || item.Name_of_Society__c || '—'}</Text>
            ...
            <View style={s.chipRow}>
              ...
              {!!item.category && <InfoChip icon="grade" label={item.category} accent={ACCENT} />}
            </View>
        ...
        ListEmptyComponent={
          <View style={s.noResults}>
            ...
            <Text style={s.noResultsText}>No RWA records found</Text>
          </View>
        }
      />
    </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.detailHero, { borderTopColor: ACCENT }]}>
          <View style={[s.detailHeroIcon, { backgroundColor: LIGHT }]}>
            <MaterialIcons name="apartment" size={32} color={ACCENT} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={s.detailHeroName}>{selected?.name_of_society || '—'}</Text>
            <View style={s.chipRow}>
              {!!selected?.city && <InfoChip icon="location-city" label={selected.city} accent={ACCENT} />}
              {!!selected?.category && <InfoChip icon="grade" label={selected.category} accent={ACCENT} />}
              {!!selected?.delivery_mode && <InfoChip icon="local-shipping" label={selected.delivery_mode} accent={ACCENT} />}
            </View>
          </View>
        ...
        <View style={s.detailCard}>
          <View style={[s.detailCardBar, { backgroundColor: ACCENT }]}>
            <MaterialIcons name="location-on" size={13} color="#fff" style={{ marginRight: 5 }} />
            <Text style={s.detailCardBarText}>LOCATION DETAILS</Text>
          </View>
          <SFRow L="Name of Society" LV={selected?.name_of_society} R="Address" RV={selected?.address} />
          <SFRow L="Depot" LV={selected?.depot_name} R="Locality" RV={selected?.locality_name} />
          <SFRow L="City" LV={selected?.city} R="Pincode" RV={selected?.locality_pincode} />
          <SFRow L="Branch" LV={selected?.branch_name} R="Delivery Mode" RV={selected?.delivery_mode} />
        </View>

        {/* Section: Property */}
        <View style={s.detailCard}>
          <View style={[s.detailCardBar, { backgroundColor: ACCENT }]}>
            <MaterialIcons name="home-work" size={13} color="#fff" style={{ marginRight: 5 }} />
            <Text style={s.detailCardBarText}>PROPERTY DETAILS</Text>
          </View>
          <SFRow L="Category" LV={selected?.category} R="Premiumness" RV={selected?.premiumness} />
          <SFRow L="Slab (per sq ft)" LV={selected?.slab} R="Total Flats" RV={selected?.total_flats?.toString()} />
          <SFRow L="Occupied Flats" LV={selected?.occupied_flats?.toString()} R="" RV="" />
        </View>
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[s.formTopBar, { backgroundColor: ACCENT }]}>
        ...
        <Text style={s.formTopBarText}>{isUpdate ? 'Edit RWA Record' : 'New RWA Record'}</Text>
      </View>
      <ScrollView style={s.page} keyboardShouldPersistTaps="handled">
        <View style={s.formBody}>
          ...
          <TextFieldInput label="Name of Society" required value={form.name_of_society} onChangeText={v => setField('name_of_society', v)} />
          <TextFieldInput label="Address" required value={form.address} onChangeText={v => setField('address', v)} multiline />
          <TextFieldInput label="Slab (per sq ft rate)" value={form.slab} onChangeText={v => setField('slab', v)} placeholder="e.g. 4000-5000" />
          <TextFieldInput label="Premiumness" value={form.premiumness} onChangeText={v => setField('premiumness', v)} keyboardType="numeric" />
          <DropdownPicker label="Category" value={form.category} options={CATEGORY_OPTIONS} onSelect={v => setField('category', v)} />
          <DropdownPicker label="City" value={form.city} options={CITY_OPTIONS} onSelect={v => setField('city', v)} />

          <Text style={[s.formSectionLabel, { color: ACCENT }]}>Depot & Locality</Text>
          <SearchPicker
            label="Depot" required placeholder="Search depots..."
            ...
            onSelect={r => r
              ? setForm(p => ({ ...p, depot_id: r.id, depot_name: r.name, branch_id: r.rmd_branch_id || '', branch_name: r.branch_name || '' }))
              : setForm(p => ({ ...p, depot_id: '', depot_name: '', branch_id: '', branch_name: '' }))}
          />
          <SearchPicker
            label="Locality" required placeholder="Search localities..."
          ...
          <Text style={[s.formSectionLabel, { color: ACCENT }]}>Flat Details</Text>
          <TextFieldInput label="Total no. of Flats" value={form.total_flats} onChangeText={v => setField('total_flats', v)} keyboardType="numeric" />
          <TextFieldInput label="No. of flats occupied" value={form.occupied_flats} onChangeText={v => setField('occupied_flats', v)} keyboardType="numeric" />
          <DropdownPicker label="Delivery Mode" value={form.delivery_mode} options={DELIVERY_OPTIONS} onSelect={v => setField('delivery_mode', v)} />
