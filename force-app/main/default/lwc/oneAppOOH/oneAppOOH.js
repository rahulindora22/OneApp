...
import viewTransactionDetails from '@salesforce/apex/SegmentControllerOOH.viewTransactionDetails';
import viewTransactionDetailsSecondTime from '@salesforce/apex/SegmentControllerOOH.viewTransactionDetailsSecondTime';
...
export default class OneAppLwc extends NavigationMixin(LightningElement) {
    ...
    @wire(getPicklistValues,

       {
          ...
          // recordTypeId: '0129D000001B93EQAS', 
   ...
   connectedCallback(){
       ...
       console.log('accountMetadata-'+JSON.stringify(this.accountMetadata.data));
         ...
         console.log('pickval-'+this.locationService);
      }
    ...
    @track closeExitOOH= true;
    @track isOOHUpdate = false;
    updateOOH(){
        this.closeExitOOH = false;
        this.isOOHUpdate = true;
      }

      handleSuccessOOH(event){
        this.dispatchEvent(
            new ShowToastEvent({
                title: 'success',
        ...
        this.isOOHUpdate = false;
        this.closeExitOOH = true;
    }
...
//handleClickRwa in RWA
    ...
    onBackClickRWA(){
        ...
        this.dispatchEvent(new CustomEvent('oohback'));
    ...
    handleSubmit1(event){
        console.log('OHH Update handleSubmit1() ');
    }
...
handleClickLink(event){
    ...
    getSegmentContacts({ segmentIdforSegContacts :  this.segmentIdforSC}) 
           .then(results => { 
            console.log('Ashish true record Id 1st = '+JSON.stringify(results)); 

                   this.contList = results;  
                   displaySegmentContactVendor({segmentIdforSegCntcts : this.segContId})
                   .then(results => { 
                    console.log('Ashish true record Id 2nd = '+JSON.stringify(results)); 
...
saveRecord(event){ 
   ...
   if(this.newRecordView == true) {
    ...
    saveSegmentContacts({
        ...
        .then(result => {
            console.log('--result save con ven----@@@@'+JSON.stringify(result));
            this.message = result;
            this.error = undefined;
            if(this.message != undefined) {
                this.ShowRowSC = false;
...
handleRowActionV1(event){
    ...
    getCurrentBrowserLocationOn() {
        ...
        this.recordTypeRWACheckin ='OOH';
         console.log('=this.recordTypeRWACheckin='+this.recordTypeRWACheckin);
         console.log('= this.segmentIdforSC='+ this.segmentIdforSC);
         this.getGeoLocationNew();
          console.log('=Rahul=');
         var flag = true; 
        ...
        if(navigator.geolocation) {  
                ...
                navigator.geolocation.getCurrentPosition((position)=> {
                    ...
                    if(position.coords.latitude != null && position.coords.longitude != null){
                        latCI = position.coords.latitude;
                        langCI = position.coords.longitude;
                    }
                    var segIdNew ;
                    var recTypeOHHCI ;

                    if(this.segmentIdforSC != null && this.segmentIdforSC != ''&& this.segmentIdforSC != undefined){
                        segIdNew = this.segmentIdforSC;
                    }
                    if(this.recordTypeRWACheckin != null && this.recordTypeRWACheckin != ''&& this.recordTypeRWACheckin != undefined){
                        recTypeOHHCI = this.recordTypeRWACheckin;
                    }
                    console.log('= latCI=',latCI);
                    console.log('= langCI', langCI);
                    console.log('= segIdNew=',segIdNew);
                    console.log('=recTypeOHHCI=', recTypeOHHCI);

                    /*checkInSecondTime({
                        lat: latCI, lng: langCI, recordId : segIdNew,
                        recordTypeCheckin : recTypeOHHCI
                    })
                        ...
                        if(result == 'success'){
                            ...
                            this.dispatchEvent(
                                new ShowToastEvent({
                                    ...
                                    message: 'Your check in location not in meter. Please check in Range',
                                    variant: 'error'
                                })
                            );
                            this.isRendered = false;      
        ...
        console.log('>>> in checkOutFromOneApp');
            ...
            if (locationService && locationService.isAvailable()) {
                locationService.getCurrentPosition({ enableHighAccuracy: true })
                    ...
                    .catch(error => {
                        console.error('Salesforce Location Error', error);
                        this.getBrowserLocationFallback();
                    });
        } 
        else {
            console.log('Indora123===');
            if (!navigator.geolocation) {
         ...
         navigator.geolocation.getCurrentPosition(
            ...
            },
            {
                ...
                maximumAge: 0
                    ...
                    checkOutFromOneApp({
                        lat: lats, lng: lngs, recordId : this.segmentId,
                    ...
                    .then(result => {
                        ...
                        this.dispatchEvent(
                            new ShowToastEvent({
                                title: 'OOH Check-Out Successful!!',
                                variant: 'success'
                            })
                        );
   ...
   console.log('Indora===');
    ...
    // Salesforce Location Service (BEST)
    const locationService = getLocationService();

    if (locationService && locationService.isAvailable()) {
        locationService.getCurrentPosition({ enableHighAccuracy: true })
            .then(position => {
                this.handleCheckIn(position.coords.latitude, position.coords.longitude);
            })
            .catch(error => {
                console.error('Salesforce Location Error', error);
                this.getBrowserLocationFallback();
...
handleCheckIn(lat, lng) {
    if (!lat || !lng) {
        this.showLocationError();
        return;
    }
    console.log('= latCI@=',lat);
    console.log('= langCI@=', lng);
    console.log('= this.segmentIdforSC@=',this.segmentIdforSC); 
    console.log('= this.recordTypeRWACheckin@=', this.recordTypeRWACheckin);
    checkInSecondTime({
        lat: lat,
        lng: lng,
        recordId: this.segmentIdforSC,
        ...
        if (result === 'success') {
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    ...
                    message: 'Please check in within allowed distance.',
                    variant: 'error'
                })
            );
        }
